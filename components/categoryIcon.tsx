...
const [displacementPosition, setDisplacementPosition] = useState(0);
    ...
    useEffect(() => {
        const handleMouseMove = (e:any) => {
         constIconPosition = document.getElementById(`categoryIcon${position}`)?.getBoundingClientRect();
  ...
  return (
    <div id={`categoryIcon${position}`} style={{transform:`translateY(${displacementPosition}rem)`, transitionDelay:`${position/3}s`, transitionProperty:"transform", transitionTimingFunction:"ease-out", transitionDuration:"200ms"}} className={`text-primary10 flex flex-col font-semibold items-center justify-center h-[80px] laptop:h-[90px] desktop:h-[100px] w-[80px] laptop:w-[90px] desktop:w-[100px] hover:bg-gradient-to-t from-primary1 to-primary2 rounded-[50%] shadow-2xl cursor-pointer border-[1px] border-primary4 transition-all`}>
